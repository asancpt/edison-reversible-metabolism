...
```{r infusion_history}
S1 <- data.frame(TIME=c(0, 60), RATE1=c(2,0), RATE2=c(0.1, 0))
S2 <- data.frame(TIME=c(0,5), RATE1=c(0, 0), RATE2=c(4, 0))
...
```{r}
as.list(final_fit$Est[1,])

final_fit$Est[1, c('Vc', 'Vm', 'CLp', 'CLm', 'CLd1', 'CLd2')]
...
```{r}
fit_data <- bind_rows(raw_dPK34, fit_dPK34) %>% 
  mutate(scenario = case_when(
    ID==1 ~ 'IV infusion (60min) of parent (120mg)\n + metabolite (6mg)',
    ID==2 ~ 'IV infusion (5min) of metabolite (20mg)'
  )) %>% 
